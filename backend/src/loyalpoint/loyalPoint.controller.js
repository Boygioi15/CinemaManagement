...
class LoyalPointController {
  ...
  resetLoyalPoint = expressAsyncHandler(async (req, res) => {
    ...
    const response = await LoyalPointService.resetLoyalPoint(customerId, newPointValue);
    return res.status(200).json({
  ...
  addLoyalPoint = expressAsyncHandler(async (req, res) => {
    ...
    const response = await LoyalPointService.addLoyalPoint(customerId, pointsToAdd);
    return res.status(200).json({
